      ...
      <header className="border-b-2 border-black px-4 md:px-8 py-4 md:py-5">
        <div className="max-w-screen-2xl mx-auto flex items-center justify-between lg:justify-start gap-4 lg:gap-10">
          ...
          <Link to="/" className="shrink-0 flex flex-col">
            <span className="font-black uppercase leading-none" style={{ fontSize: 24, letterSpacing: '-0.05em' }}>
              Naija Mart
            </span>
          ...
          <div className="hidden lg:flex items-center gap-8 shrink-0">
            ...
            <Link to="/cart" className="flex flex-col items-start relative">
              ...
              <span className="text-[13px] font-bold mt-0.5">Cart ({totalItems})</span>
            </Link>
          </div>

          <div className="flex lg:hidden items-center gap-4 shrink-0">
            <Link to="/cart" className="text-[12px] font-bold">Cart ({totalItems})</Link>
            <button
        ...
        <div className="lg:hidden max-w-screen-2xl mx-auto mt-3 flex border-2 border-black">
          <input type="text" placeholder="Search products…" className="flex-1 px-3 py-2 text-sm outline-none bg-transparent" />
          <button className="px-4 py-2 bg-black text-white text-[10px] tracking-[0.2em] uppercase font-bold shrink-0">Go</button>
        </div>
          ...
          {NAV_CATEGORIES.map(cat => (
            <button
              ...
              className={`shrink-0 px-4 py-3 text-[10px] tracking-[0.18em] uppercase font-semibold border-b-2 whitespace-nowrap ${
                activeNav === cat ? 'border-black text-black' : 'border-transparent text-neutral-500 hover:text-black'
              }`}
            >
              {cat}
            </button>
